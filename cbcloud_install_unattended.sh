...
function set_upgrade_config() {
  if [[ ${install_upgrade} == 'upgrade' ]]; then
...
function setup() {
  ...
  # Clear upgrade indicator before we've determined whether or not upgrade is occurring
  rm -f "${tmp_cfg_ini}.upg"
  ...
  # defer network entension enablement handling flag
  if [[ ${DEFER_NE_ENABLEMENT} -eq 0 ]] ; then
    echo "Option: defer network extension is disable."
    echo "DeferNetworkExtensionEnablement=false" >> "${tmp_cfg_ini}"
  fi
  ...
  if [[ -n "${ENABLE_FIPS}" ]]; then
    if [[ "${ENABLE_FIPS}" -gt "0" ]]; then
      ...
      echo "Option: Disabling FIPS mode"
      echo "FipsMode=false" >> "${tmp_cfg_ini}"
    fi
    set_upgrade_config
  fi
